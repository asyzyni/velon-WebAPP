...
import { Search, Users, Fuel, Settings, Calendar, X } from 'lucide-react';
import BookingModal from './BookingModal';
import { getAllCars, searchAvailableCars, type BackendCar } from '../api/carApi';
...
export default function CarCatalog() {
  ...
  const mapBackendCars = (data: BackendCar[]): Car[] => {
    return data.map((car: BackendCar) => ({
      id: car.id.toString(),
      name: car.namaMobil,
  ...
  const fetchCars = async () => {
    try {
      if (startDate && endDate) {
        ...
        const data = await searchAvailableCars(startDate, endDate);
        setCars(mapBackendCars(data));
      } else {
    ...
    <div>
      <div className="bg-white rounded-xl shadow-md p-6 mb-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          <h2 className="text-gray-900">Cari Mobil</h2>
          {isSearchingDates && (
            <div className="flex items-center gap-2 text-sm text-blue-600 bg-blue-50 px-3 py-1.5 rounded-full">
              <span>Menampilkan mobil tersedia: {startDate} s/d {endDate}</span>
              <button onClick={handleClearDates} className="hover:text-blue-800">
                <X className="w-4 h-4" />
              </button>
            </div>
          )}
